...
# === Run PCA for all datasets and haplotypes ===
all_results <- list()
...
# === Combine all PCA points for distribution ===
...
# === Overall PCA distribution plot ===
dist_plot <- ggplot(pca_points, aes(x = PC1, y = PC2, color = Dataset, shape = Haplotype)) +
  geom_point(alpha = 0.5, size = 1.5) +
  facet_wrap(~ Dataset, scales = "free") +
  labs(title = "PCA Distribution Across All Datasets", x = "PC1", y = "PC2") +
  theme_minimal() +
  theme(
...
# === Run MANOVA and generate plots per dataset ===
manova_results <- list()

for (ds in datasets) {
  df_subset <- pca_points %>% filter(Dataset == ds)
  manova_out <- manova(cbind(PC1, PC2) ~ Haplotype, data = df_subset)
  manova_text <- paste(capture.output(summary(manova_out)), collapse = "\n")
  
  # Create MANOVA ellipse plot
  ellipse_plot <- ggplot(df_subset, aes(x = PC1, y = PC2, color = Haplotype)) +
    geom_point(alpha = 0.6, size = 2) +
    stat_ellipse(type = "norm", level = 0.95, linetype = "dashed", size = 1) +
    labs(title = paste("MANOVA Ellipse -", ds),
         x = "PC1", y = "PC2") +
    theme_minimal()
  
  manova_results[[ds]] <- list(summary = manova_text, plot = ellipse_plot)
}
...
# Add MANOVA summaries and plots
doc <- doc %>% body_add_par("MANOVA Summary & Plots", style = "heading 1")

for (ds in names(manova_results)) {
  doc <- doc %>%
    body_add_par(paste("Dataset:", ds), style = "heading 2") %>%
    body_add_par(manova_results[[ds]]$summary, style = "Normal") %>%
    body_add_gg(value = manova_results[[ds]]$plot, width = 6, height = 5)
}
